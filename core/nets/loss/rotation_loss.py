...
class RotationLoss(nn.Module):
    ...
    def forward(
        ...
        # Only anchors assigned to the spacecraft contribute
        # to rotation supervision.
        positive_indices = torch.eq(
            anchor_states,
            1,
        )

        # A valid differentiable zero protects against a rare
        # batch containing no positive anchors.
        if not positive_indices.any():
            return r_raw_pr.sum() * 0.0

        # Convert predicted continuous 6D rotations into
        # proper 3x3 rotation matrices.
        r6d = r_raw_pr[
            positive_indices,
            :,
        ]
        ...
        # Repeat each image-level ground-truth rotation for
        # all anchors, then retain only positive anchors.
        R_gt = R_gt.view(
            batch_size,
            1,
            3,
        ...
        # Relative rotation between prediction and ground truth.
        R_relative = torch.bmm(
            R_pr,
            R_gt.transpose(1, 2),
        )

        trace = R_relative.diagonal(
            offset=0,
            dim1=1,
            dim2=2,
        ).sum(-1)
        ...
        # Clamping avoids unstable gradients at exactly -1 or 1.
